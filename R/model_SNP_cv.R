...
model_SNP_cv <- function(out_data){
  ...
  SNP_phe           <-  t(round(myData2,2)) ##每一行为一个小鼠样本，每一列为一个SNP，最后一列为表型
  ...
  if(TRUE){
    model_cv <- cv.glmnet(SNP_data,phe_data,family = "gaussian",type.measure = "mse",nfolds = 10)

    y_predict <- predict(model_cv,newx = SNP_data,s = "lambda.min")
    sst <- sum((phe_data-mean(phe_data))^2)
    sse <- sum((y_predict-phe_data)^2)
    R2 <- 1-sse/sst

    coef_num <- as.matrix(coef(model_cv,s="lambda.min"))
    coef_num <- coef_num[coef_num!=0]
    coef_num <- coef_num[-1]
    adjusted_R2 <- 1- (sse/(dim(phe_data)[1]-length(coef_num)-1))/(sst/(dim(phe_data)[1]-1))
    return(adjusted_R2)
  } ### 仅十折交叉验证 ，输出为一个数字。
}
